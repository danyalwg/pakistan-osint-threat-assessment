...
def ensure_default_keyword_files(base_dir: str) -> None:
    ...
    (Your gui can still overwrite them with richer content.)
    """
...
def _compile_keyword_patterns(keywords: List[str]) -> List[Tuple[str, re.Pattern]]:
    ...
    for k in _clean_keywords(keywords):
        # Alnum-only => word boundaries reduce false positives
        if re.fullmatch(r"[A-Za-z0-9]+", k):
            pat = re.compile(rf"\b{re.escape(k)}\b", flags=re.IGNORECASE)
        else:
            # phrase or contains punctuation / non-latin => substring match
...
def shortlist_articles_two_layer(
    ...
    """
    Layer 1 (National):
      Article must match >=1 national keyword to pass.

    Layer 2 (Threat):
      Only applied to Layer-1-passed articles.
      Article must match >=1 threat keyword to pass.

    Fields written into each Article:
      - keywords_national_matched, keywords_threat_matched
      - national_relevant, threat_relevant, shortlisted
      - keywords_matched (backward compatible union)
    """
    ...
    for a in articles:
        ...
        a.keywords_national_matched = nat_hits
        a.national_relevant = bool(nat_hits)
        nat_hits_total += len(nat_hits)

        # clear layer 2 initially (so old values don't linger)
        a.keywords_threat_matched = []
        a.threat_relevant = None
        a.shortlisted = None

        # backward compatible union bucket
        # (keep older code working: treat national hits as keywords_matched baseline)
        a.keywords_matched = list(dict.fromkeys((a.keywords_matched or []) + nat_hits))

        if a.national_relevant:
    ...
    for a in national_pass:
        ...
        a.keywords_threat_matched = thr_hits
        a.threat_relevant = bool(thr_hits)
        thr_hits_total += len(thr_hits)

        # final shortlisted decision (your described logic)
        a.shortlisted = bool(a.national_relevant and a.threat_relevant)

        # keep keywords_matched union updated for legacy UI fields (KW hits)
        a.keywords_matched = list(dict.fromkeys((a.keywords_matched or []) + thr_hits))
